...
typedef struct {
    ...
    uint64_t last_burst_reset_ms; // FIX: Tiene traccia della finestra assoluta dei 60s
...
float compute_nelo_friction(friction_state_t *state, float d_raw, uint64_t now_ms) {
    ...
    const float M = 11985.0f;      // Pendenza calibrata per L(1.0) -> ~3600s
    ...
    if (state->last_update_ms == 0) {
        ...
        state->last_burst_reset_ms = now_ms;
    }

    // Calcolo dt reale rispetto all'ultima invocazione
    float dt = (now_ms - state->last_update_ms) / 1000.0f;
    if (dt < 0.0f) dt = 0.0f; // Protezione da anomalie di clock o overflow

    // FIX: Reset del burst count basato su una finestra temporale assoluta di 60s
    if ((now_ms - state->last_burst_reset_ms) / 1000.0f > 60.0f) {
        state->burst_count = 0;
        state->last_burst_reset_ms = now_ms;
    }
    ...
    // Calcolo Cooldown Adattivo
    float burst_factor = (state->burst_count > 2) ? 5.0f : 0.0f;
    float tau = TAU_BASE * (1.0f + ALPHA * burst_factor);
    
    // Aggiornamento D_internal con decadimento ed inserimento nuovo danno
    state->d_internal = state->d_internal * expf(-dt / tau);
    state->d_internal = fmaxf(state->d_internal, d_raw);
    
    // FIX: Salva il timestamp corrente per il prossimo ciclo dt
    state->last_update_ms = now_ms;

    float d = state->d_internal;
    if (d <= 0.05f) return 0.0f;

    // Calcolo della funzione a tratti
    float delay;
    if (d <= 0.7f) {
        delay = L_BASE * expf(K1 * d);
    } else {
        // Calcolo continuo: valore esponenziale a 0.7 + incremento lineare
        float exp_junction = L_BASE * expf(K1 * 0.7f);
        delay = exp_junction + M * (d - 0.7f);
    }
